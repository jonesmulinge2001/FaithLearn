import { EnrolledCourse } from './../interfaces/interfaces';
...
export class LessonsService {
  ...
  getCourse(id: string): Observable<Course> {
    return this.http.get<Course>(
      `${this.baseUrl}/lessons/courses/${id}`,
      this.getHeaders()
  ...
  ): Observable<StudentQuestion> {
    return this.http.post<StudentQuestion>(
  ...
  getQuestionsByLesson(lessonId: string): Observable<StudentQuestion[]> {
    return this.http.get<StudentQuestion[]>(
  ...
  getAllStudentQuestions(): Observable<StudentQuestion[]> {
    return this.http.get<StudentQuestion[]>(
      `${this.baseUrl}/lessons/questions/all`,
      this.getHeaders()
    );
  }

  replyToStudentQuestion(
    questionId: string,
    answer: string
  ): Observable<StudentQuestion> {
    return this.http.post<StudentQuestion>(
      `${this.baseUrl}/lessons/questions/reply/${questionId}`,
      { answer },
      this.getHeaders()
    );
  }
  ...
  submitStudentAnswer(
    ...
    return this.http.post<StudentAnswer>(
      `${this.baseUrl}/lessons/answers/${studentId}/${questionId}`,
      { answer },
      this.getHeaders()
    );
  }
  ...
  getStudentAnswers(studentId: string): Observable<StudentAnswer[]> {
    return this.http.get<StudentAnswer[]>(
      `${this.baseUrl}/lessons/student/${studentId}/answers`,
      this.getHeaders()
    );
  }

  replyToStudentAnswer(
    answerId: string,
    reply: string
  ): Observable<StudentAnswer> {
    return this.http.post<StudentAnswer>(
  ...
  enrollStudent(studentId: string, courseId: string): Observable<any> {
    return this.http.post(
      `${this.baseUrl}/lessons/enroll/${studentId}/${courseId}`,
      {},
      this.getHeaders()
    );
  }

  getEnrolledCourses(studentId: string): Observable<EnrolledCourse[]> {
    return this.http.get<EnrolledCourse[]>(
      `${this.baseUrl}/lessons/enrolled-courses/${studentId}`,
      this.getHeaders()
    );
  }
}
